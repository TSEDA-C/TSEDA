...
double runNGA(string XmlFile, string RscAlcFile, double& SchTime, int& iteration){
    ...
    population[0] = GnrChr_HEFT_b_t(Rank_b_t);
    population[1] = GnrChr_HEFT_t(Rank_t);
    population[2] = GnrChr_HEFT(Rank_b);
#pragma omp parallel for
    ...
    int terminationNum = ceil(50 * sqrt(ModelScale)/Parameter_NGA.NumOfChromPerPop);
    int NumOfNoImpGen = 0;
    ...
    while(1) {
        ...
        #pragma omp parallel for
        for (int n = 0; n < Parameter_NGA.NumOfChromPerPop; ++n) {
            GnrML_Evl_EFT(NextPopulation[n]);
