...
//        if(fabs(ld) <= 1e-1) {
//            return ld*46.7 + 9.33;
//        } else if(fabs(ld-0.1) <= 1e-1) {
//            return ld*32 + 10.8;
//        } else if(fabs(ld-0.2) <= 1e-1) {
//            return ld*27 + 11.8;
//        } else if(fabs(ld-0.3) <= 1e-1) {
//            return ld*26 + 12.1;
//        }else if(fabs(ld-0.4) <= 1e-1) {
//            return ld*30 + 10.5;
//        }else if(fabs(ld-0.5) <= 1e-1) {
//            return ld*40 + 5.5;
//        } else if(fabs(ld-0.6) <= 1e-1) {
//            return ld*51 - 1.1;
//        } else if(fabs(ld-0.7) <= 1e-1) {
//            return ld*63 - 9.5;
//        }else if(fabs(ld-0.8) <= 1e-1) {
//            return ld*70 - 15.1;
//        } else if(fabs(ld-0.9) <= 1e-1) {
//            return ld*39 + 12.8;
//        }
//    } else {
//        if(fabs(ld) <= 1e-1) {
...
int SearchRsc(int& task, int& Rsc){
    int min = 0; int max = Tasks[task].ElgRsc.size();
    if(Rsc < Tasks[task].ElgRsc[0]) return Tasks[task].ElgRsc[0];
    if(Rsc > Tasks[task].ElgRsc[Tasks[task].ElgRsc.size()-1]) return Tasks[task].ElgRsc[Tasks[task].ElgRsc.size()-1];
    while (min < max - 1){
        int mid = (max + min) / 2;
        if(Tasks[task].ElgRsc[mid] > Rsc){
            max = mid;
        } else if(Tasks[task].ElgRsc[mid] < Rsc){
            min = mid;
        }
    }
    int left = abs(Tasks[task].ElgRsc[min] - Rsc);
    int right = abs(Tasks[task].ElgRsc[max] - Rsc);
    if(left < right){
        return Tasks[task].ElgRsc[min];
    } else if(left > right){
        return Tasks[task].ElgRsc[max];
    } else {
        int rdm = rand() % 2;
        if(left == right && rdm == 0) return Tasks[task].ElgRsc[min];
        else return Tasks[task].ElgRsc[max];
    }
}
