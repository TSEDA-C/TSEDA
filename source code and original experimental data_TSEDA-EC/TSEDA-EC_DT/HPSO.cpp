...
double runHPSO(string XmlFile, string RscAlcFile, double& SchTime, int& iteration){
    ...
    population.push_back(GnrPrtByRank_EFT(Rank_b1));
    population.push_back(GnrPrtByRank_EFT(Rank_t));

    for(int i = 1; i < Parameter_HPSO.NumOfChromPerPop; ++i){
        population.push_back(GnrPrtByRank_Rnd(Rank_b1));
        population.push_back(GnrPrtByRank_Rnd(Rank_t));
    }
    ...
    int terminationNum = ceil(50 * sqrt(ModelScale)/Parameter_HPSO.NumOfChromPerPop); double TotalNum = terminationNum;
