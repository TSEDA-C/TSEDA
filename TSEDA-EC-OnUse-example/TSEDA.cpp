...
chromosome runTSEDA(string XmlFile, string RscAlcFile,double& SchTime, int& iteration) {
    clock_t start = clock();
...
//    #pragma omp parallel for
    ...
    while (RunTime + PrecisionValue < Parameter_TSEDA.RunTimeRatioOfStg1 * SchTime) {
        ...
        if (BstChrom.EnergyConsumption < 2199.20001) {
    ...
    while (RunTime + PrecisionValue < SchTime){
        ...
        #pragma omp parallel for
        for (int n = 0; n < Parameter_TSEDA.NumOfImproveOfPop; ++n) {//when finding cases, use "NumOfChromPerPop"
