...
//        if (fabs(Tasks[i].IFileSizeSum - Tasks[i].OrignalInputFileSizeSum) > PrecisionValue) {
//            cout << endl << i << ": " << Tasks[i].IFileSizeSum << "; " << Tasks[i].OrignalInputFileSizeSum;
//        }
    }

    //{Rsc can be added here}
    //{configuration 4 for example}
    Resource Rsc_0 = Resource(0, 2, 2000);
    Resource Rsc_1 = Resource(0, 4, 4000);
    Resource Rsc_2 = Resource(0, 4, 4000);

    Rscs.push_back(Rsc_0);
    Rscs.push_back(Rsc_1);
    Rscs.push_back(Rsc_2);
//    Rscs.push_back(Rsc_3);
    vector<int> VMlist0 = {0, 1, 2};
...
void ConfigParameter_TSEDA() {
    ...
    Parameter_TSEDA.RunTimeRatioOfStg1 = 0.75;
}
...
void ConfigParameter_ADBRKGA() {
    ...
    Parameter_ADBRKGA.beta = 0.8; //0.8
    Parameter_ADBRKGA.BiasesRate= 0.75;
    Parameter_ADBRKGA.ImmigrationRate = 0.07; //0.07
