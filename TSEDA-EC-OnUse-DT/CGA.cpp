...
chromosome runCGA(string XmlFile, string RscAlcFile, double& SchTime, int& iteration) {
    clock_t start = clock();
...
//#pragma omp parallel for
    ...
    int terminationNum = ceil(CDT * sqrt(ModelScale)/Parameter_CGA.NumOfChromPerPop);
    int NumOfNoImpGen = 0;
