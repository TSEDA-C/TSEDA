...
chromosome runHPSO(string XmlFile, string RscAlcFile, double& SchTime, int& iteration){
    clock_t start = clock();
...
//    C_Cal_Average(cc);                           //calculate the average transfer time among tasks
    ...
    int terminationNum = ceil(CDT * sqrt(ModelScale)/Parameter_HPSO.NumOfChromPerPop); double TotalNum = terminationNum;
    ...
    while (1){
        ...
        for(int i = 0; i < Parameter_HPSO.NumOfChromPerPop; ++i){
            ...
            DcdEvl_S(population[i]);
            CalculateEnergy1(population[i]);
        }
        ++NumOfNoImpGen;
        for (int i = 0; i < Parameter_HPSO.NumOfChromPerPop; ++i) {
